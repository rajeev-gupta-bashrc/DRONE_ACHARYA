...
if sim_version == 1:
    ...
    print(
        "Please assign only 1 or 2 to the variable 'sim_version' in the input function"
    )
    exit()

# Generate the refence signals
t = np.arange(
    0, 100 + Ts * innerDyn_length, Ts * innerDyn_length
)  # time from 0 to 100 seconds, sample time (Ts=0.4 second)
...
(
    ...
    Z_dot_dot_ref,
    psi_ref,
) = support.trajectory_generator(t)
...
statesTotal = [
    states
]  # It will keep track of all your states during the entire manoeuvre
# statesTotal2=[states]
...
for i_global in range(0, plotl - 1):
    ...
    phi_ref, theta_ref, U1 = support.pos_controller(
        X_ref[i_global + 1],
        X_dot_ref[i_global + 1],
        X_dot_dot_ref[i_global + 1],
        Y_ref[i_global + 1],
        Y_dot_ref[i_global + 1],
        Y_dot_dot_ref[i_global + 1],
        Z_ref[i_global + 1],
        Z_dot_ref[i_global + 1],
        Z_dot_dot_ref[i_global + 1],
        psi_ref[i_global + 1],
        states,
    )
    ...
    for i in range(0, innerDyn_length):
        ...
        (
            ...
            psi,
            psi_dot,
        ) = support.LPV_cont_discrete(states, omega_total)
        ...
        velocityXYZ_total = np.concatenate(
            (velocityXYZ_total, temp_velocityXYZ), axis=0
        )
        # Generate the augmented current state and the reference vector
        x_aug_t = np.transpose(
            [
                np.concatenate(
                    ([phi, phi_dot, theta, theta_dot, psi, psi_dot], [U2, U3, U4]),
                    axis=0,
                )
            ]
        )
        ...
        omegas_bundle = np.concatenate(
            (omegas_bundle, np.array([[omega1, omega2, omega3, omega4]])), axis=0
        )

        # Compute the new total omega
        omega_total = omega1 - omega2 + omega3 - omega4
        # Compute new states in the open loop system (interval: Ts/10)
        states, states_ani, U_ani = support.open_loop_new_states(
            states, omega_total, U1, U2, U3, U4
        )
        # print(states)
        # print(statesTotal)
...
length_x = (
    max_ref * 0.15
)  # Length of one half of the UAV in the x-direction (Only for the animation purposes)
length_y = (
    max_ref * 0.15
)  # Length of one half of the UAV in the y-direction (Only for the animation purposes)
...
def update_plot(num):
    ...
    R_x = np.array(
        [
            [1, 0, 0],
            [0, np.cos(statesTotal_phi[num]), -np.sin(statesTotal_phi[num])],
            [0, np.sin(statesTotal_phi[num]), np.cos(statesTotal_phi[num])],
        ]
    )
    R_y = np.array(
        [
            [np.cos(statesTotal_theta[num]), 0, np.sin(statesTotal_theta[num])],
            [0, 1, 0],
    ...
    R_z = np.array(
        [
            [np.cos(statesTotal_psi[num]), -np.sin(statesTotal_psi[num]), 0],
            [np.sin(statesTotal_psi[num]), np.cos(statesTotal_psi[num]), 0],
            [0, 0, 1],
        ]
    )
    ...
    drone_body_x.set_xdata(
        [
            statesTotal_x[num] + drone_pos_inertial_x_neg[0][0],
            statesTotal_x[num] + drone_pos_inertial_x[0][0],
        ]
    )
    drone_body_x.set_ydata(
        [
            statesTotal_y[num] + drone_pos_inertial_x_neg[1][0],
            statesTotal_y[num] + drone_pos_inertial_x[1][0],
        ]
    )

    drone_body_y.set_xdata(
        [
            statesTotal_x[num] + drone_pos_inertial_y_neg[0][0],
            statesTotal_x[num] + drone_pos_inertial_y[0][0],
        ]
    )
    drone_body_y.set_ydata(
        [
            statesTotal_y[num] + drone_pos_inertial_y_neg[1][0],
            statesTotal_y[num] + drone_pos_inertial_y[1][0],
        ]
    )
    ...
    drone_body_x.set_3d_properties(
        [
            statesTotal_z[num] + drone_pos_inertial_x_neg[2][0],
            statesTotal_z[num] + drone_pos_inertial_x[2][0],
        ]
    )
    drone_body_y.set_3d_properties(
        [
            statesTotal_z[num] + drone_pos_inertial_y_neg[2][0],
            statesTotal_z[num] + drone_pos_inertial_y[2][0],
        ]
    )

    if sim_version == 1:
        drone_body_phi.set_data(
            [-length_y * 0.9 * 0.9, length_y * 0.9 * 0.9],
            [drone_pos_inertial_y_neg[2][0], drone_pos_inertial_y[2][0]],
        )
        drone_body_theta.set_data(
            [length_x * 0.9 * 0.9, -length_x * 0.9 * 0.9],
            [drone_pos_inertial_x[2][0], drone_pos_inertial_x_neg[2][0]],
        )
        ...
        return (
            ...
            drone_position_y,
            drone_position_z,
            drone_orientation_phi,
            drone_orientation_theta,
            drone_orientation_psi,
        )
...
if sim_version == 1:
    ...
    (drone_body_phi,) = ax1.plot(
        [], [], "--g", linewidth=2, label="drone_y (+: Z-up,Y-right,phi-CCW)"
    )
    ...
    (drone_body_theta,) = ax2.plot(
        [], [], "--r", linewidth=2, label="drone_x (+: Z-up,X-left,theta-CCW)"
    )
    ...
    ax5.plot(
        t_angles, ref_angles_total[:, 1], "b", linewidth=1, label="Theta_ref [rad]"
    )
...
drone_ani = animation.FuncAnimation(
...
if no_plots == 1:
    ...
    ax.set_zlabel("Z [m]")
    copyright = ax.text(
        0, max(Y_ref), max(Z_ref) * 1.20, "© Mark Misin Engineering", size=15
    )
    ax.legend()
    plt.show()
    ...
    plt.plot(
        t,
        velocityXYZ_total[0 : len(velocityXYZ_total) : innerDyn_length, 0],
        "r",
        linewidth=1,
        label="X_dot",
    )
    ...
    plt.plot(
        t,
        velocityXYZ_total[0 : len(velocityXYZ_total) : innerDyn_length, 1],
        "r",
        linewidth=1,
        label="Y_dot",
    )
    ...
    plt.plot(
        t,
        velocityXYZ_total[0 : len(velocityXYZ_total) : innerDyn_length, 2],
        "r",
        linewidth=1,
        label="Z_dot",
    )
    ...
    plt.plot(
        t_angles,
        omegas_bundle[0 : len(omegas_bundle), 0],
        "b",
        linewidth=1,
        label="omega 1",
    )
    ...
    plt.plot(
        t_angles,
        omegas_bundle[0 : len(omegas_bundle), 1],
        "b",
        linewidth=1,
        label="omega 2",
    )
    ...
    plt.plot(
        t_angles,
        omegas_bundle[0 : len(omegas_bundle), 2],
        "b",
        linewidth=1,
        label="omega 3",
    )
    ...
    plt.plot(
        t_angles,
        omegas_bundle[0 : len(omegas_bundle), 3],
        "b",
        linewidth=1,
        label="omega 4",
    )
