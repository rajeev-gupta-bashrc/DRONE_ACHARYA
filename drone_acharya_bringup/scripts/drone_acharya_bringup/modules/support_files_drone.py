...
class SupportFilesDrone:
    ...
    def __init__(self):
        ...
        # Constants (Iris drone)
        Ix = 0.008  # kg*m^2
        Iy = 0.015  # kg*m^2
        Iz = 0.017  # kg*m^2
        m = 1.5  # kg
        g = 9.81  # m/s^2
        # l = 0.2555386468 #m distance of rotor from center
        # Jtp = 1.302 * 10 ** (-6)  # N*m*s^2=kg*m^2
        # Jtp = 0.1149500000  # N*m*s^2=kg*m^2
        Jtp = 0.000670416  # N*m*s^2=kg*m^2
        Ts = 0.1  # s
        ...
        r_in = 10
        Q = np.matrix(
            f"{cost} 0 0;0 {cost} 0;0 0 {cost}"
        )  # weights for outputs (all samples, except the last one)
        S = np.matrix(
            f"{s_out} 0 0;0 {s_out} 0;0 0 {s_out}"
        )  # weights for the final horizon period outputs
        R = np.matrix(f"{r_in} 0 0;0 {r_in} 0;0 0 {r_in}")  # weights for inputs
        # # Matrix weights for the cost function (They must be diagonal)
        # Q = np.matrix(
        ...
        ct = 0.00007435892055/4  # N*s^2
        # cq = 2.6839 * 10 ** (-9) * (60 / (2 * np.pi)) ** 2  # N*m*s^2
        cq = 0.00000002608733668  # N*m*s^2
        # l = 0.171  # m
        l = 0.2555386468  # m
        ...
        innerDyn_length = 6  # Number of inner control loop iterations
        ...
        no_plots = (
            0  # 0-you will see the plots; 1-you will skip the plots (only animation)
        )
    ...
    def trajectory_generator(self, t):
        ...
        if trajectory == 1 or trajectory == 2 or trajectory == 3 or trajectory == 4:
            ...
            if trajectory == 2:
                ...
                z_dot[101 : len(z_dot * (t / 20))] = (
                    d_height / (t[-1]) * np.ones(len(t[101 : len(t)]))
                )
                ...
                y[101 : len(y)] = 2 * (t[101 : len(t)] - t[100]) / 20 + y[100]
                z[101 : len(z)] = (
                    z[100] + d_height / t[-1] * (t[101 : len(t)] - t[100]) ** 2
                )

                x_dot[101 : len(x_dot)] = 1 / 10 * np.ones(len(t[101 : len(t)]))
                y_dot[101 : len(y_dot)] = 1 / 10 * np.ones(len(t[101 : len(t)]))
                z_dot[101 : len(z_dot)] = (
                    2 * d_height / (t[-1]) * (t[101 : len(t)] - t[100])
                )

                x_dot_dot[101 : len(x_dot_dot)] = 0 * np.ones(len(t[101 : len(t)]))
                y_dot_dot[101 : len(y_dot_dot)] = 0 * np.ones(len(t[101 : len(t)]))
                z_dot_dot[101 : len(z_dot_dot)] = (
                    2 * d_height / t[-1] * np.ones(len(t[101 : len(t)]))
                )
                ...
                x[101 : len(x)] = 2 * (t[101 : len(t)] - t[100]) / 20 + x[100]
                y[101 : len(y)] = 2 * (t[101 : len(t)] - t[100]) / 20 + y[100]
                z[101 : len(z)] = z[100] + 50 * d_height / t[-1] * np.sin(
                    0.1 * (t[101 : len(t)] - t[100])
                )

                x_dot[101 : len(x_dot)] = 1 / 10 * np.ones(len(t[101 : len(t)]))
                y_dot[101 : len(y_dot)] = 1 / 10 * np.ones(len(t[101 : len(t)]))
                z_dot[101 : len(z_dot)] = (
                    5 * d_height / t[-1] * np.cos(0.1 * (t[101 : len(t)] - t[100]))
                )

                x_dot_dot[101 : len(x_dot_dot)] = 0 * np.ones(len(t[101 : len(t)]))
                y_dot_dot[101 : len(y_dot_dot)] = 0 * np.ones(len(t[101 : len(t)]))
                z_dot_dot[101 : len(z_dot_dot)] = (
                    -0.5 * d_height / t[-1] * np.sin(0.1 * (t[101 : len(t)] - t[100]))
                )
            ...
            if power == 1:
                ...
                x_dot = (
                    r_2 * np.cos(alpha) - (r_2 * t + r) * np.sin(alpha) * 2 * np.pi * f
                )
                y_dot = (
                    r_2 * np.sin(alpha) + (r_2 * t + r) * np.cos(alpha) * 2 * np.pi * f
                )
                z_dot = d_height / (t[-1]) * np.ones(len(t))

                x_dot_dot = (
                    -r_2 * np.sin(alpha) * 4 * np.pi * f
                    - (r_2 * t + r) * np.cos(alpha) * (2 * np.pi * f) ** 2
                )
                y_dot_dot = (
                    r_2 * np.cos(alpha) * 4 * np.pi * f
                    - (r_2 * t + r) * np.sin(alpha) * (2 * np.pi * f) ** 2
                )
                z_dot_dot = 0 * np.ones(len(t))
            else:
                # Trajectory 6
                ...
                x_dot = (
                    power * r_2 * t ** (power - 1) * np.cos(alpha)
                    - (r_2 * t ** (power) + r) * np.sin(alpha) * 2 * np.pi * f
                )
                y_dot = (
                    power * r_2 * t ** (power - 1) * np.sin(alpha)
                    + (r_2 * t ** (power) + r) * np.cos(alpha) * 2 * np.pi * f
                )
                z_dot = d_height / (t[-1]) * np.ones(len(t))

                x_dot_dot = (
                    power * (power - 1) * r_2 * t ** (power - 2) * np.cos(alpha)
                    - power * r_2 * t ** (power - 1) * np.sin(alpha) * 2 * np.pi * f
                ) - (
                    power * r_2 * t ** (power - 1) * np.sin(alpha) * 2 * np.pi * f
                    + (r_2 * t**power + r_2) * np.cos(alpha) * (2 * np.pi * f) ** 2
                )
                y_dot_dot = (
                    power * (power - 1) * r_2 * t ** (power - 2) * np.sin(alpha)
                    + power * r_2 * t ** (power - 1) * np.cos(alpha) * 2 * np.pi * f
                ) + (
                    power * r_2 * t ** (power - 1) * np.cos(alpha) * 2 * np.pi * f
                    - (r_2 * t**power + r_2) * np.sin(alpha) * (2 * np.pi * f) ** 2
                )
                z_dot_dot = 0 * np.ones(len(t))
    ...
    def pos_controller(
        ...
        if Psi_ref >= 0:
            Psi_ref_singularity = (
                Psi_ref - np.floor(abs(Psi_ref) / (2 * np.pi)) * 2 * np.pi
            )
        else:
            Psi_ref_singularity = (
                Psi_ref + np.floor(abs(Psi_ref) / (2 * np.pi)) * 2 * np.pi
        ...
        if (
            np.abs(Psi_ref_singularity) < np.pi / 4
            or np.abs(Psi_ref_singularity) > 7 * np.pi / 4
        ) or (
            np.abs(Psi_ref_singularity) > 3 * np.pi / 4
            and np.abs(Psi_ref_singularity) < 5 * np.pi / 4
        ):
    ...
    def mpc_simplification(self, Ad, Bd, Cd, Dd, hz):
        ...
        for i in range(0, hz):
            if i == hz - 1:
                Qdb[
                    np.size(CSC, 0) * i : np.size(CSC, 0) * i + CSC.shape[0],
                    np.size(CSC, 1) * i : np.size(CSC, 1) * i + CSC.shape[1],
                ] = CSC
                Tdb[
                    np.size(SC, 0) * i : np.size(SC, 0) * i + SC.shape[0],
                    np.size(SC, 1) * i : np.size(SC, 1) * i + SC.shape[1],
                ] = SC
            else:
                Qdb[
                    np.size(CQC, 0) * i : np.size(CQC, 0) * i + CQC.shape[0],
                    np.size(CQC, 1) * i : np.size(CQC, 1) * i + CQC.shape[1],
                ] = CQC
                Tdb[
                    np.size(QC, 0) * i : np.size(QC, 0) * i + QC.shape[0],
                    np.size(QC, 1) * i : np.size(QC, 1) * i + QC.shape[1],
                ] = QC

            Rdb[
                np.size(R, 0) * i : np.size(R, 0) * i + R.shape[0],
                np.size(R, 1) * i : np.size(R, 1) * i + R.shape[1],
            ] = R

            for j in range(0, hz):
                if j <= i:
                    Cdb[
                        np.size(B_aug, 0) * i : np.size(B_aug, 0) * i + B_aug.shape[0],
                        np.size(B_aug, 1) * j : np.size(B_aug, 1) * j + B_aug.shape[1],
                    ] = np.matmul(
                        np.linalg.matrix_power(A_aug, ((i + 1) - (j + 1))), B_aug
                    )

            Adc[
                np.size(A_aug, 0) * i : np.size(A_aug, 0) * i + A_aug.shape[0],
                0 : 0 + A_aug.shape[1],
            ] = np.linalg.matrix_power(A_aug, i + 1)
    ...
    def open_loop_new_states(self, states, omega_total, U1, U2, U3, U4):
        ...
        for j in range(0, 4):
            ...
            w_dot = (
                (u * q - v * p) - g * np.cos(theta) * np.cos(phi) + U1 / m - Fd_w / m
            )
            ...
            if j < 3:
                ...
                u = (
                    u_or
                    + 1 / 6 * (u_dot_k1 + 2 * u_dot_k2 + 2 * u_dot_k3 + u_dot_k4) * Ts
                )
                v = (
                    v_or
                    + 1 / 6 * (v_dot_k1 + 2 * v_dot_k2 + 2 * v_dot_k3 + v_dot_k4) * Ts
                )
                w = (
                    w_or
                    + 1 / 6 * (w_dot_k1 + 2 * w_dot_k2 + 2 * w_dot_k3 + w_dot_k4) * Ts
                )
                p = (
                    p_or
                    + 1 / 6 * (p_dot_k1 + 2 * p_dot_k2 + 2 * p_dot_k3 + p_dot_k4) * Ts
                )
                q = (
                    q_or
                    + 1 / 6 * (q_dot_k1 + 2 * q_dot_k2 + 2 * q_dot_k3 + q_dot_k4) * Ts
                )
                r = (
                    r_or
                    + 1 / 6 * (r_dot_k1 + 2 * r_dot_k2 + 2 * r_dot_k3 + r_dot_k4) * Ts
                )
                x = (
                    x_or
                    + 1 / 6 * (x_dot_k1 + 2 * x_dot_k2 + 2 * x_dot_k3 + x_dot_k4) * Ts
                )
                y = (
                    y_or
                    + 1 / 6 * (y_dot_k1 + 2 * y_dot_k2 + 2 * y_dot_k3 + y_dot_k4) * Ts
                )
                z = (
                    z_or
                    + 1 / 6 * (z_dot_k1 + 2 * z_dot_k2 + 2 * z_dot_k3 + z_dot_k4) * Ts
                )
                phi = (
                    phi_or
                    + 1
                    / 6
                    * (phi_dot_k1 + 2 * phi_dot_k2 + 2 * phi_dot_k3 + phi_dot_k4)
                    * Ts
                )
                theta = (
                    theta_or
                ...
                psi = (
                    ...
                    + 1
                    / 6
                    * (psi_dot_k1 + 2 * psi_dot_k2 + 2 * psi_dot_k3 + psi_dot_k4)
                    * Ts
                )
        ...
        for k in range(0, sub_loop):
            ...
            states_ani[k, 4] = (
                theta_or + (theta - theta_or) / Ts * (k / (sub_loop - 1)) * Ts
            )
